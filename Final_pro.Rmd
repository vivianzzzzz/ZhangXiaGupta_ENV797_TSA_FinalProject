...
```
``` {r}
#Neural network with fourier (3,12),p=1,P=0
NN_fit2 <- nnetar(msts_bitcoin,p=3,P=1,xreg=fourier(msts_bitcoin, K=c(3,12)))
NN_for2 <- forecast(NN_fit1,h=266, xreg=fourier(msts_bitcoin, K=c(3,12),h=266))
autoplot(NN_for2)
autoplot(msts_bitcoin) +
  autolayer(NN_for2, series="Neural Network",PI=FALSE)

NN_scores2 <- accuracy(NN_for2$mean,msts_bitcoin_test)
print(NN_scores2)
```
```{r}
...
scores <- rbind(ETS_scores, TBATS_scores, ARIMA_scores,ARIMA_scores1, NN_scores1,NN_scores2,NN_scores3,SSES_scores1)
rownames(scores) <- c("ETS", "TBATS", "ARIMA","ARIMA with Covid","Neural Network1","Neural Network2","Neural Network with Covid","SSES")
print(scores)
